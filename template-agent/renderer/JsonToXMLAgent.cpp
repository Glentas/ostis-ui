...
ScResult JsonToXMLAgent::DoProgram(ScActionInitiatedEvent const & event, ScAction & action)
{
  ...
  {
    SC_THROW_EXCEPTION(utils::ExceptionItemNotFound, "Link not found");
  }
...
std::string JsonToXMLAgent::escapeXml(std::string const & str)
{
  std::string result;
...
// Очистка имени тега
std::string JsonToXMLAgent::sanitizeTagName(std::string const & name)
{
  if (name.empty())
    return "item";
...
std::string JsonToXMLAgent::jsonToXml(nlohmann::json const & j, std::string const & tagName)
{
...
std::string JsonToXMLAgent::convertJsonToXml(std::string const & jsonStr)
{
  try
  {
